...
This guide documents how the Rocky Linux 9 OpenSSL FIPS provider
...
dlsym(3).
    ...
    const OSSL_RL_FIPSINDICATOR_ALORITHM *(*rocky_ossl_query_fipsindicator)(int) \
        = dlsym(provider, "rocky_ossl_query_fipsindicator");
    if (rocky_ossl_query_fipsindicator == NULL) {
        fprintf(stderr, "%s\n", dlerror());
...
I<rocky_ossl_query_fipsindicator> expects an operation ID as its only
...
I<algorithm> contains the algorithm name to search for:
    ...
    const OSSL_RL_FIPSINDICATOR_ALGORITHM *indicator =
        rocky_ossl_query_fipsindicator(operation_id);
    if (indicator == NULL) {
